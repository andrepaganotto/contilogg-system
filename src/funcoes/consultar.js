 ...
 * @property {"fill"|"click"|"upload"|"select"|"press"} action
 * @property {string} selector
 * @property {string=} key
 * @property {{ role?: string|null, text?: string|null, networkTriggered?: boolean, key?: string, resultSelector?: boolean }=} meta
 *
 * @typedef {Object} Mapa
...
/**
 * Executa uma consulta navegando por um mapa (modo "consultar") e retorna true/false
 * conforme a presença do step marcado com meta.resultSelector === true.
 *
 * @param {Object} params
 ...
 */
async function consultar({ url, loginInfo, dados = {}, mapa, options = {} }) {
    const {
        headless = false,
        timeoutMs = 15000,
        postClickWaitMs = 500,
        typeDelayMs = 50,
        resultWaitMs = 4000, // quanto esperar para achar o resultSelector
    } = options;
    ...
    // Validação de steps que exigem dados/arquivos
    const missing = [];
    const missingFiles = [];
    let resultFound = false;
    for (const step of mapa.steps) {
        if (!step || !step.action || !step.selector)
            throw new Error('Step inválido: cada step precisa de "action" e "selector".');
  ...
  `;
    ...
    // Digitação "humana" e Enter no mesmo campo
    ...
    async function openMenuAncestorsFor(handle, { hoverDelay = 150, clickDelay = 120, maxHops = 6 } = {}) {
        ...
        for (let i = ancestors.length - 1; i >= 0; i--) {
            const a = ancestors[i];
            try { await a.scrollIntoViewIfNeeded(); } catch { }
    ...
    async function robustClick({ selector, meta }) {
        ...
        if (isVisibleNow) {
            try { await clickable.scrollIntoViewIfNeeded(); } catch { }
    ...
    try {
        ...
        for (let i = 0; i < mapa.steps.length; i++) {
            ...
            // Se este step marca o "resultSelector", apenas verificamos a presença e
            // marcamos o resultado, mas NÃO retornamos ainda (vamos deslogar antes).
            if (meta && meta.resultSelector === true) {
                try {
                ...
                break; // o "resultSelector" é o último passo relevante; vamos para o logout
            }
        ...
        // LOGOUT ao final (se presente no mapa), independente do resultado encontrado
        if (mapa.logout) {
            try {
